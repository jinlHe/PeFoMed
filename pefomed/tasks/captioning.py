...
from pefomed.common.dist_utils import main_process, get_rank
from pefomed.common.registry import registry
from pefomed.tasks.base_task import BaseTask
from pefomed.common.utils import is_convertible_to_int, is_url, cache_url
...
@registry.register_task("medcaptioning")
class MEDCaptionTask(BaseTask):
    def __init__(self, num_beams, max_len, min_len, repetition_penalty, length_penalty, top_p, temperature, evaluate, report_metric=True, annotation_file=None, sample_id_key="image_id", caption_key="caption", split=["val"], load_gt_from_file=False, img_ids = []):
        super().__init__()
    ...
    def build_datasets(self, cfg):
        ...
        if not val_ds_name:
            return datasets # no validation sets
        assert len(val_ds_name) == 1, "Only support one dataset for validation"
        val_ds_name = val_ds_name[0] #"iuxray"

        # get question file, annotation file and anwser list in COCO format
        if self.annotation_file == None:
            if 'coco' not in val_ds_name: # coco is already precomputed in dataset
                self.annotation_file = os.path.join(registry.get_path("cache_root"),f'{val_ds_name}_gt', f'{val_ds_name}_{self.split}_annotations.json') #iuxray_val_annotations.json
                if get_rank() == 0:
                    if not os.path.isfile(self.annotation_file):
                        os.makedirs(os.path.join(registry.get_path("cache_root"),f'{val_ds_name}_gt'), exist_ok=True)
                        convert_to_coco_gt(datasets[val_ds_name], self.annotation_file, self.caption_key, self.sample_id_key, self.split, load_gt_from_file=self.load_gt_from_file, img_ids=self.img_ids)
                    else:
                        print('Using existing file ' + self.annotation_file)
                if "test" in datasets[val_ds_name]:
                    annotation_file_test = os.path.join(registry.get_path("cache_root"), f'{val_ds_name}_gt',f'{val_ds_name}_test_annotations.json')  # iuxray_test_annotations.json
                    if get_rank() == 0:
                        if not os.path.isfile(annotation_file_test):
                            os.makedirs(os.path.join(registry.get_path("cache_root"), f'{val_ds_name}_gt'), exist_ok=True)
                            convert_to_coco_gt(datasets[val_ds_name], annotation_file_test, self.caption_key,self.sample_id_key, split="test", load_gt_from_file=self.load_gt_from_file, img_ids=self.img_ids)
                        else:
                            print('Using existing file ' + annotation_file_test)
        return datasets
    ...
    def after_evaluation(self, val_result, split_name, epoch, **kwargs):
        ...
        if self.report_metric:
            # print("eval_result_file:", eval_result_file)  #/mnt/sda/hjl/weights/test2/result/test_epochbest.json
            metrics = self._report_metrics(
    ...
    def _report_metrics(self, eval_result_file, split_name, epoch):
        # print("annotation_file:", self.annotation_file) #/mnt/sda/hjl/.cache/pefomed/iuxray_gt/iuxray_val_annotations.json
        if self.annotation_file == None:
            # TODO better way to define this
            coco_gt_root = os.path.join(registry.get_path("cache_root"), "coco_gt")
            coco_val = coco_caption_eval(coco_gt_root, eval_result_file, split_name, img_ids=self.img_ids)
            ...
            if split_name =="test":
                self.annotation_file = self.annotation_file.replace("val", "test")
            coco_val = coco_caption_eval(None, eval_result_file, split_name, annotation_file=self.annotation_file, img_ids=self.img_ids)

        agg_metrics = coco_val.eval["CIDEr"] + coco_val.eval["Bleu_4"]
        log_stats = {split_name: {k: v for k, v in coco_val.eval.items()}}
        log_stats[split_name]['epoch'] = epoch
...
def convert_to_coco_gt(data, outpath, caption_key, sample_id_key, split, load_gt_from_file=False, img_ids=[]):
    ...
    if load_gt_from_file:
        ...
        for i,ann in tqdm(enumerate(data[split])):
            if isinstance(data[split].annotation, dict):
                image_keys = list(data[split].annotation.keys())
                img_key = image_keys[i]
                captions = data[split].annotation[img_key][0]
            else:
